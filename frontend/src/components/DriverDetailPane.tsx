...
export function DriverDetailPane({
  ...
  const driver = selectedDriver ? (results.drivers.find((d) => d.abbr === selectedDriver) ?? null) : null;
  const driverInsight = selectedDriver ? insights.find((d) => d.id === selectedDriver) : undefined;
  const driverStints = selectedDriver
    ? (stints?.drivers.find((d) => d.abbr === selectedDriver)?.stints ?? [])
    : [];
  ...
  return (
    <section
      ...
      style={{
        borderLeftColor: resolveTeamColor(driver?.team ?? "Unknown"),
        borderLeftWidth: "4px",
      }}
    >
      {circuit && (
      ...
      <div className="mt-4 min-h-[280px] rounded-inner border border-line bg-panel/50 p-3">
        {!driver && (
          <>
        ...
        {driver && (
          <>
            <h3 className="text-card-heading font-medium text-primary">{driver.full_name ?? driver.abbr}</h3>
            ...
            {driverInsight && (
              <>
                ...
                <details className="mt-3 rounded-inner border border-line bg-panel/50 p-3">
                  ...
                  <div className="mt-3">
                    <PerformanceSignature profile={driverInsight.profile} profileRanks={driverInsight.profile_ranks} />
                  </div>
